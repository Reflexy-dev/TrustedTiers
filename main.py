...
def update_json_file(guild: discord.Guild):
    ...
    if GITHUB_TOKEN and REPO_NAME:
        ...
        print("⚠️ GITHUB_TOKEN o GITHUB_REPO non impostati nelle variabili d'ambiente.")

# --- GENERAZIONE EMBED E UPDATER CODA ---
...
# --- MODALI CODA MINECRAFT ---
class MinecraftNameModal(discord.ui.Modal):
    def __init__(self, gamemode: str):
        super().__init__(title=f"Queue: {gamemode}")
        self.gamemode = gamemode

        self.mc_name = discord.ui.TextInput(label="Minecraft Username", placeholder="Ex: Steve...", required=True)
        self.region = discord.ui.TextInput(label="Region", placeholder="EU, NA, AS...", default="EU", required=True, max_length=10)
...
class StandardResultModal(discord.ui.Modal):
    def __init__(self, player_member, mc_name, gamemode, ticket_channel_id, region):
        ...
        self.new_rank = discord.ui.TextInput(label="New Rank Earned (LT5 to LT3)", placeholder="Ex: LT3", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if rank_earned not in TIER_ORDER or TIER_ORDER.index(rank_earned) > TIER_ORDER.index("LT3"):
            await interaction.response.send_message(f"❌ The rank `{rank_earned}` belongs to High Tiers! Please use High Tier system.", ephemeral=True)
            return
...
class HighTierResultModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if rank_earned not in allowed_high_result_tiers:
            await interaction.response.send_message("❌ In a High Tier Test, lowest assigned result is **LT3**!", ephemeral=True)
            return
...
class StaffControlView(discord.ui.View):
    ...
    async def next_player_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if existing_channel:
            await interaction.response.send_message(f"❌ A match channel already exists for this player ({existing_channel.mention})!", ephemeral=True)
            return
...
class HighStaffControlView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if not has_tester_role or not has_lt3:
            await interaction.response.send_message(
                f"❌ To join as High Tester, you must have **Tester {self.gamemode}** role AND be at least **LT3 {self.gamemode}**!",
                ephemeral=True
            )
            return False
        return True
    ...
    async def next_player_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        existing_channel = discord.utils.get(guild.text_channels, name=expected_channel_name)
        if existing_channel:
            await interaction.response.send_message(f"❌ A high match channel already exists for this player ({existing_channel.mention})!", ephemeral=True)
            return
...
@bot.tree.command(name="syncjson", description="Sincronizza manualmente il file data.json su GitHub")
@app_commands.default_permissions(administrator=True)
async def syncjson(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
...
async def on_ready():
    ...
    try:
        ...
        print(f"Errore durante lo sync dei comandi: {e}")

# --- MINI SERVER WEB INTEGRATO ---
...
if __name__ == "__main__":
    ...
    if TOKEN:
        ...
        print("❌ DISCORD_TOKEN non trovato nelle variabili d'ambiente!")
